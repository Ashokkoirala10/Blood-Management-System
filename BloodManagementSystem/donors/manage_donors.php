...
// Total donor count
$total_result = mysqli_query($conn, "SELECT COUNT(*) AS total FROM donors");
$total_row = mysqli_fetch_assoc($total_result);
$total_donors = $total_row['total'];
$total_pages = ceil($total_donors / $records_per_page);

// Fetch donor records
$result = mysqli_query($conn, "SELECT * FROM donors LIMIT $offset, $records_per_page");
...
<head>
    <meta charset="UTF-8">
    <title>Manage Donors - BloodLink</title>
...
<main class="container mt-5 flex-grow-1">
    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap">
        <h3 class="mb-3 text-white ">Donor List</h3>
        <a href="add_donor.php" class="btn add-btn"><i class="fas fa-plus"></i> Add Donor</a>
    </div>
        ...
        <table class="table table-striped table-bordered align-middle text-center" >
            ...
            <?php while($row = mysqli_fetch_assoc($result)) { ?>
                <tr>
                    ...
                    <td>
                        <span class="badge bg-<?= $row['availability'] ? 'success' : 'secondary' ?>">
                            <?= $row['availability'] ? 'Yes' : 'No' ?>
                        </span>
                    </td>
                    <td>
                        <?php 
                            $photoPath = '../uploads/donor_photos/' . $row['profile_photo'];
                            if (!empty($row['profile_photo']) && file_exists($photoPath)): ?>
                                <img src="<?= htmlspecialchars($photoPath) ?>" alt="Profile Photo" width="50" height="50">
                            <?php else: ?>
                                <span class="text-muted">No Photo</span>
                            <?php endif; ?>
                    </td>
                    <td>
                        <a href="update.php?user_id=<?= $row['user_id'] ?>" class="btn btn-sm btn-primary">
                            <i class="fas fa-edit"></i>
                        </a>
                        <?php if ($_SESSION['role'] === 'admin'): ?>
                            <a href="delete_donor.php?user_id=<?= $row['user_id'] ?>" 
